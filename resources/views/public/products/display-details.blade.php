...
@section('content')
            ...
            <div class="w-full max-width-container-bordered bg-white p-4">
                <div class="flex flex-col lg:flex-row">
                    <div class="w-full lg:w-2/6 flex flex-col items-stretch justify-start h-64">
                        @include('public.partials.imageviewer', ['printerphotos' => $display->getAllPhotoUrls()])
                    </div>
                    <div class="w-full lg:w-3/6 flex flex-col items-stretch justify-start h-64">
                        ...
                        <div class="w-full py-4 flex flex-col items-start justify-center">
                            @if($display->product_url_on_manufacturer_website != null)
                                <a target="_blank" class="text-blue-500 hover:underline hover:text-blue-700" href="{{ $display->product_url_on_manufacturer_website }}">Leírás</a>
                            @endif
                            @if($display->specification_sheet != null)
                                <a target="_blank" class="text-blue-500 hover:underline hover:text-blue-700" href="{{ $display->specification_sheet }}">Technikai adatok</a>
                            @endif
                        </div>
                    </div>
        ...
        <div class="w-full max-width-container-bordered bg-white p-4 flex flex-col hidden transition-visible"  id="contact-form-container">
            <div class="flex flex-row items-start justify-start py-4 w-full bg-white">
                @include('public.partials.contactform', [
                    ...
                    'defaultMessage' => 'Tárgy: kérdés a(z) '.$display->displayname.' termékkel kapcsolatban'."\n\n"
                ])
            </div>
        </div>
            ...
            <div class="flex flex-row items-start justify-start py-4 w-full bg-white">
                ...
                >
                    ...
                    @include('public.partials.formelements.text-input', [
                        ...
                        'label' => 'Tárgy',
                        'mandatory' => false,
                        'value' => $display->displayname.' - Fotorex.hu'
                    ])
    ...
    <form method="post" action="{{ route('print_to_pdf', ['slug' => $display->slug]) }}" id="print-to-pdf">
        @csrf
    </form>
    <script>

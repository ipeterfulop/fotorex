<style>
</style>
<div x-data="listOrGridView_{{ $componentId }}()" x-init="() => {initialize()}" class="w-full flex flex-col lg:flex-row p-2 ajax-list-or-grid">
    @if(count($filters) > 0)
        <div class="w-full md:w-1/3">
            @include('public.partials.list-or-grid-filters')
        </div>
    @endif
    <div class="flex flex-col w-full @if(count($filters) > 0) md:w-2/3 @endif">
        <div class="h-auto lg:h-12 flex flex-start flex-wrap lg:flex-no-wrap flex-row p-4 lg:p-0 lg:pl-2 mb-0 items-center justify-start bg-fotomediumgray" id="top-{{ $componentId }}">
            <label class="mr-2">Rendezés:</label>
            <select x-model="sortingOption">
                @foreach($sortingOptions as $id => $label)
                    <option value="{{ $id }}">{{ $label }}</option>
                @endforeach
            </select>
            <label class="hidden lg:block ml-4">Megjelenítés:</label>
            <button class="hidden lg:flex flex-row items-center justify-start ml-4 hover-red-link pr-2 focus:outline-none" @click="switchToList()" x-bind:class="{'opacity-25': containerClass != 'fotorex-list-container'}">
                <span class="h-8 w-8 mr-1">{!! config('heroicons.solid.view-list') !!}</span>Lista
            </button>
            <button class="hidden lg:flex flex-row items-center justify-start ml-4 hover-red-link pr-2 focus:outline-none" @click="switchToGrid()" x-bind:class="{'opacity-25': containerClass != 'fotorex-grid-container'}">
                <span class="h-8 w-8 mr-1">{!! config('heroicons.solid.view-grid') !!}</span>Rács
            </button>
        </div>
        <div x-show.transition.opacity="loading" class="w-full p-4 my-32 flex justify-center items-center"><img style="max-height: 1em" alt="" src="data:image/svg+xml;base64," /></div>
        <div class="flex w-full"
             x-show.transition.opacity="!loading"
             x-bind:class="containerClass"
             x-html="content"
        >
        </div>
    </div>

</div>
<script>
    function listOrGridView_{{ $componentId }}() {
        return {
            loaderSrc: '<div class="w-full p-4 my-32 flex justify-center items-center"><img style="max-height: 1em" alt="" src="data:image/svg+xml;base64," /></div>',
            content: '',
            scroll: false,
            ajaxURL: new URL('{{ $url }}'),
            baseURL: window.location.href,
            sortingOption: '{{ array_keys($sortingOptions)[0] }}',
            containerClass: 'fotorex-list-container',
            page: {{ $page }},
            filters: {
                @foreach($filters as $filter)
                    {{ $filter->getField() }}: '{{ $filter->getValue() }}',
                @endforeach
            },
            loading: true,
            resetUrl() {
                this.ajaxURL = new URL('{{ $url }}');
            },
            initFilters(params) {
                if (typeof(this.$refs.filters) != 'undefined') {
                    Object.keys(params).forEach((key) => {
                        this.ajaxURL.searchParams.set(key, params[key]);
                    });
                    Array.from(this.$refs.filters.querySelectorAll('.fotorex-filter')).forEach((filter) => {
                        if (filter.getAttribute('data-filtertype') == 'text') {
                            if (params[filter.getAttribute('data-field')]) {
                                filter.querySelector('.fotorex-filter-text').value = params[filter.getAttribute('data-field')];
                            }
                        }
                        if (filter.getAttribute('data-filtertype') == 'range') {
                            if (params[filter.getAttribute('data-field')]) {
                                let pieces = params[filter.getAttribute('data-field')].split('-');
                                filter.querySelector('.range-value-input').value = params[filter.getAttribute('data-field')];
                                filter.querySelector('.price-slider-min-input').value = pieces[0];
                                filter.querySelector('.price-slider-max-input').value = pieces[1];
                                filter.querySelector('.price-slider-update-button').click();
                            }
                        }
                        if (filter.getAttribute('data-filtertype') == 'radiogroup') {
                            if (params[filter.getAttribute('data-field')]) {
                                filter.querySelector('.fotorex-filter-radiogroup[name="'+filter.getAttribute('data-field')+'"][value="'+params[filter.getAttribute('data-field')]+'"]').setAttribute('checked', true)
                            }
                        }
                        if (filter.getAttribute('data-filtertype') == 'checkboxgroup') {
                            if (params[filter.getAttribute('data-field')]) {
                                let currentFilter = params[filter.getAttribute('data-field')].split(',');
                                Array.from(filter.querySelectorAll('.fotorex-filter-checkboxgroup')).forEach((item) => {
                                    item.checked = currentFilter.indexOf(item.value) > -1;
                                });
                            } else {
                                Array.from(filter.querySelectorAll('.fotorex-filter-checkboxgroup')).forEach((item) => {
                                    item.checked = false;
                                });
                            }
                        }
                    });
                }
            },
            pushState: {{ $pushState ? 'true' : 'false' }},
            initialize() {
                if (window.localStorage) {
                    let savedView = window.localStorage.getItem('listOrGridViewState');
                    if (savedView == 'list') {
                        this.switchToList();
                    }
                    if (savedView == 'grid') {
                        this.switchToGrid();
                    }
                }
                this.$watch('sortingOption', (value) => {this.loadContent(true)});
               this.initFilters(Object.fromEntries(new URL(window.location.href).searchParams.entries()));
                this.loadContent(true);
                if (this.pushState) {
                    window.addEventListener('popstate', (event) => {
                        this.page = event.state.page;
                        this.sortby = event.state.sortby;
                        this.initFilters(event.state.params);

                    })
                }
            },
            storeState(state) {
                if (window.localStorage) {
                    window.localStorage.setItem('listOrGridViewState', state);
                }
            },
            switchToList() {
                this.containerClass = 'fotorex-list-container'
                this.storeState('list');
                this.initPaginationLinks();
            },
            switchToGrid() {
                this.containerClass = 'fotorex-grid-container';
                this.storeState('grid');
                this.initPaginationLinks();
            },
            showPage(p) {
                if (this.page != p) {
                    this.page = p;
                    this.loadContent(false);
                }
            },
            initPaginationLinks() {
                this.$nextTick(() => {
                    Array.from(document.querySelectorAll('.fotorex-list-container-pagination-link')).forEach((link) => {
                        link.addEventListener('click', (event) => {
                            this.showPage(link.getAttribute('data-page'));
                        })
                    })
                })
            },
            loadContent(resetPage) {
                this.loading = true;
                if (resetPage) {
                    this.page = 1;
                }
                this.resetUrl();
                this.ajaxURL.searchParams.set('sortby', this.sortingOption);
                this.ajaxURL.searchParams.set('page', this.page);
                if (typeof(this.$refs.filters) != 'undefined') {
                    Array.from(this.$refs.filters.querySelectorAll('.fotorex-filter')).forEach((filter) => {
                        let value = -1;
                        if (filter.getAttribute('data-filtertype') == 'text') {
                            value = filter.querySelector('.fotorex-filter-text').value;
                        }
                        if (filter.getAttribute('data-filtertype') == 'range') {
                            value = filter.querySelector('.range-value-input').value;
                        }
                        if (filter.getAttribute('data-filtertype') == 'radiogroup') {
                            let r = filter.querySelector('input[type="radio"]:checked');
                            if (r != null) {
                                value = r.value;
                            }
                        }
                        if (filter.getAttribute('data-filtertype') == 'checkboxgroup') {
                            let currentFilter = [];
                            Array.from(filter.querySelectorAll('.fotorex-filter-checkboxgroup')).forEach((item) => {
                                if (item.checked) {
                                    currentFilter.push(item.value);
                                }
                            })
                            if (currentFilter.length > 0) {
                                value = currentFilter.join(',');
                            }
                        }
                        if (value != -1) {
                            this.ajaxURL.searchParams.set(filter.getAttribute('data-field'), value);
                        }
                    });
                }
                this.ajaxURL.searchParams.set('queryid', Math.random());
                let historyUrl = new URL(this.baseURL);
                historyUrl.search = this.ajaxURL.search;
                if (this.scroll) {
                    document.getElementById('top-{{ $componentId }}').scrollIntoView();
                }
                this.scroll = true;
                if (this.pushState) {
                    let state = {params: Object.fromEntries(this.ajaxURL.searchParams.entries())};
                    state.sortby = this.sortingOption;
                    state.page = this.page;
                    history.pushState(state, 'Fotorex - keresés', historyUrl);
                }
                window.axios.get(this.ajaxURL)
                    .then((response) => {
                        this.content = response.data;
                        this.loading = false;
                        this.initPaginationLinks();
                    })
            }
        }
    }
</script>
